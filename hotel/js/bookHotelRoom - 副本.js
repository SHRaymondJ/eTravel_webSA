					...
					if (res.code == 200) {
						if (res.infos.length > 0) {
							...
							$(".selectMorning").change(function() {
								...
								$(".selectMorningText").eq(index).text($(this).children('option:selected').val());
							})
							$(".selectEvening").change(function() {
								var index = parseInt($(this).attr("index"));
								$(".selectEveningText").eq(index).text($(this).children('option:selected').val());
							})
			...
			if (res.IsNeedCreditCard) {
				...
				$.ajax({
					...
					data: {
						...
						jsonStr: '{"request":{"Id":' + netUserId + ',"CustomerId":"' + $(".passengerLi").eq(0) +
							'","UseType":"1","Language ":"' + obtLanguage + '"}}'
					},
					success: function(data) {
						var res = JSON.parse(data);
						console.log(res);
				...
				$.ajax({
					...
					success: function(data) {
						...
						//无代订权限
							...
							oneCustomer();
							// 有审批单权限
							if (passengerJson.HasTravelRequest && !$.session.get('goOnBookOrderNo')) {
