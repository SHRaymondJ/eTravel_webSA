    ...
    $.ajax(
        {
            ...
            success: function (data) {
                ...
                res.map(function (item, index) {
                    ...
                    <div class="orderFareAmount flexRow '+ showAmount + '"><div style="width:120px;">' + get_lan('orderDetail').ticketPrice + '</div>￥<span class="mainFontColor" style="font-size:20px;">' + item.Cabins[0].FareAmount + '</span></div>\
                    <div class="orderTotalTax flexRow '+ showAmount + '"><div style="width:120px;">' + get_lan('orderDetail').tax + '</div>￥<span class="mainFontColor">' + item.Cabins[0].TotalTax + '</span></div>\
                    <div class="orderServiceFare flexRow '+ showServiceFare + '"><div style="width:120px;">' + get_lan('orderDetail').serviceFare + '</div>￥<span class="mainFontColor">' + item.Cabins[0].ServiceFare + '</span></div>\
                    <div class="orderLine"></div>\
                    <div class="orderBottomPrice"></div>\
                    <div class="ordeSavePrice"></div>\
                ...
                if (domTicketInfo.type == "oneWay" && domTicketInfo.isBottom == "1") {
                    ...
                    $(".orderBottomPrice").html('' + get_lan("orderDetail").lowestFare + '： ￥<span class="bottomColor">' + domTicketInfo.lowestPrice + '</span>');
                    $(".ordeSavePrice").html('' + get_lan("orderDetail").save + '： ￥<span class="bottomColor">' + domTicketInfo.saveFare + '</span>');
                ...
                if ((domTicketInfo.type == "roundTrip" || domTicketInfo.type == "multiple") && domTicketInfo.isBottom == "1") {
                    ...
                    $(".orderBottomPrice").eq(0).html('' + get_lan("orderDetail").lowestFare + '： ￥<span class="bottomColor">' + domTicketInfo.lowestPrice + '</span>');
                    $(".ordeSavePrice").eq(0).html('' + get_lan("orderDetail").save + '： ￥<span style="color:bottomColor">' + domTicketInfo.saveFare + '</span>');
                ...
                if ((domTicketInfo.type == "roundTrip" || domTicketInfo.type == "multiple") && domTicketInfo.isBottomReturn == "1") {
                    ...
                    $(".orderBottomPrice").eq(1).html('' + get_lan("orderDetail").lowestFare + '： ￥<span class="bottomColor">' + domTicketInfo.lowestPriceReturn + '</span>');
                    $(".ordeSavePrice").eq(1).html('' + get_lan("orderDetail").save + '： ￥<span class="bottomColor">' + domTicketInfo.saveFareReturn + '</span>');
                    ...
                    $.ajax(
                        {
                            ...
                            success: function (data) {
                                ...
                                if (!searchDomInfo.alterTicketInfo && !$.session.get('goOnBookOrderNo') && !$.session.get('TACustomerId')) {
                                    ...
                                    if (!JSON.parse($.session.get('ProfileInfo')).HasTravelRequest) {
                                        ...
                                        var jsonStr = {
                                            request: {
                                                ...
                                                companyId: searchDomInfo.alterTicketInfo.split(',')[2],
                                                customerId: customerId,
                                                remarks: "",
                                                isCopy: false,
                                                language: obtLanguage,
                                                orgOrderNo: $.session.get('changeOrderNo')
                                            }
                                        }
                                        $.ajax(
                                            {
                                                ...
                                                success: function (data) {
                                                    ...
                                                    var res = JSON.parse(data);
                                                    console.log(res);
                                                    $(".orderDetail").attr("CompanyID", searchDomInfo.alterTicketInfo.split(',')[2]);
                                                    if (res == "1") {
                                                        passengersInOrder();
                                                    }
                                                },
                                    ...
                                    $.ajax(
                                        {
                                            ...
                                            success: function (data) {
                                                ...
                                                <div class="passengerLiDiv" style="width:250px;text-align:left;padding-left:45px;box-sizing:border-box;"><span class="PassengerNameText">'+ item.CustomerName + '</span><span class="changePassengerInfo specificFontColor" index="' + index + '" customerId="' + item.CustomerID + '" style="margin-left:5px;cursor:pointer;">' + get_lan('passengerInfo').changePassengerInfo + '</span></div>\
                                ...
                                //无代订权限
                                    ...
                                    if (passengerJson.HasTravelRequest && !$.session.get('goOnBookOrderNo') && !$.session.get('TAnumber')) {
                                        ...
                                        if (domTicketInfo.type == "oneWay") {
                                            var queryKey = passengerJson.ID + ',1,' + $(".orderDetail").attr("DeparteCity") + ',' + $(".orderDetail").attr("ArriveCity") + ',' + $(".orderDetail").attr("DateStart") + ',';
                                        }
                                        else if ((domTicketInfo.type == "roundTrip" || domTicketInfo.type == "multiple")) {
                                            var queryKey = passengerJson.ID + ',1,' + $(".orderDetail").attr("DeparteCity") + ',' + $(".orderDetail").attr("ArriveCity") + ',' + $(".orderDetail").attr("DateStart") + ',' + $(".orderDetail").attr("DateReturn");
                                        }
    ...
    $.ajax(
        {
            ...
            success: function (data) {
                ...
                res.map(function (item) {
                    ...
                    '+ item.InsuranceName + '： ￥' + item.InsuranceAmount + '\
...
function passengerPop() {
    $(".PassengerPop").html('\
		...
		<div class="popType flexRow">\
		<div style="width:130px;">'+ get_lan('passengerPop').popCustomerRemind + '<span class="bottomColor">*</span></div>\
			<select name="" class="popTypeSelect">\
        ...
        <input type="text" class="popPhoneInput" disabled="disabled" maxlength="11">\
        </div>\
        <div class="popMail flexRow">\
        <div style="width:130px;">'+ get_lan('passengerPop').popMail + '<span class="bottomColor">*</span></div>\
        <input type="text" class="popMailInput" disabled="disabled">\
        </div>\
...
function totalAmount() {
    ...
    $(".totalAmountText").html('<span style="font-size:16px;">￥</span>' + (total + insurancePrice));
}
//预订机票
function clickBookBtn() {
    $(".bookTicketBtn").unbind("click").click(function () {
        if ($(".passengerLi").length == 0) {
            ...
            if (($.session.get('TAnumber') || JSON.parse($.session.get('ProfileInfo')).HasTravelRequest) && (changeOrderNo == null || changeOrderNo == undefined)) {
                var customerList = '';
...
function bookTicket() {
    ...
    function book() {
        ...
        if (domTicketInfo.type == "oneWay") {
            var otherKey = ',' + $(".passengerPhone").eq(0).text() + ',' + reasonCode + ',' + domTicketInfo.lowestPrice;
        }
        else if ((domTicketInfo.type == "roundTrip" || domTicketInfo.type == "multiple")) {
            var otherKey = ',' + $(".passengerPhone").eq(0).text() + ',' + reasonCode + '_' + reasonCodeReturn + ',' + domTicketInfo.lowestPrice + ',' + domTicketInfo.lowestPriceReturn;
        }
        var remarkKey = 'NO';
        if (!searchDomInfo.alterTicketInfo) {
            ...
            $.ajax(
                {
                    ...
                    success: function (data) {
                        ...
                        var res = JSON.parse(data);
                        console.log("BookNewCorrectForInsuranceNew" + res);
                        if (res.OrderNo != null) {
                            var isTA=1;
                            var orderFinishDetail = new OrderFinishDetail(netUserId.split('\"')[1],res.AirMainRids,orderType,obtLanguage)
                            airBookOthersPopUp(orderFinishDetail,res.OrderNo,isTA) //确认机票杂项
