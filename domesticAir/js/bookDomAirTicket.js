                    ...
                    success : function(data) {
                        ...
                        function __initIfTravelRequest(passengerID){
                            ...
                            tools.customerTravelRequest(netUserId,queryKey,function(){
                                if(!customerId){
                                    customerId = $('input[name="travelApplication"]:checked').attr('customerId');
                                }
                                $(".requestCover").remove();
                                if($.session.get('TAnumber')){
                                    var addCustomerQueryKey = $.session.get('TAnumber')+','+customerId;
                                    $.ajax({
                                        type:'post',
                                        url : $.session.get('ajaxUrl'), 
                                        dataType : 'json',
                                        data:{
                                            url: $.session.get('obtCompany')+"/SystemService.svc/AddTravelRequestCustomerPost",
                                            jsonStr:'{"id":'+netUserId+',"queryKey":"'+addCustomerQueryKey+'","remarks":"","language":"'+obtLanguage+'"}'
                                        },
                                        success : function(data) {
                                            $('body').mLoading("hide");
                                            var res = JSON.parse(data);
